...

{
    ...
    if (0 < hts_numSolidWalls)
    {
        do
        {
            ...
            if (cmp != 0)
            {
                while ((curSpan != NULL && (cmp = FP2_Compare(wallFirstCol, &curSpan->lastColumn), -1 < cmp)))
                {
...

{
    ...
    for (draw = hts_screen + hts_drawingColumn + hts_screenStride * startRow; draw <= end; draw = draw + 320)
    {
        *draw = colour;
    }
    return;
    ...
    for (draw = hts_screen + hts_drawingColumn + hts_screenStride * startRow; draw <= end; draw = draw + 320)
    {
        *draw = texpixels[((int)((calcResult2 * *piVar1 + hts_playerYFixed) & (xItMask | 0xffff)) >> 16) +
                          ((int)((calcResult1 * *piVar1 + hts_playerXFixed) & yItMask) >>
                           ((16U - (char)otherItShift) & 31)) +
                          (0 % texHeight) * texWidth];
        piVar1 = piVar1 + 1;
    }
    return;
}
